...
class nc_csv
{
    ...
    protected function export($options) {
        $component_id = nc_array_value($options, 'component_id') ?: $this->get_class_id(nc_array_value($options, 'subclass_id'));
        if (!$component_id) {
        ...
        $values_query = "SELECT " . implode(",", (array_merge($fields_in_sql, $this->default_fields))) .
                        "  FROM Message" . (int)$component_id;

        if (isset($options['subdivision_id']) && isset($options['subclass_id'])) {
            $values_query .= " WHERE Subdivision_ID = " . (int)$options['subdivision_id'] .
