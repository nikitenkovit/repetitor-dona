(function(nc){
    ...
    */
    ...
    var month_name = function(month, short){
        short = short || false;
        return config[short ? 'months_short' : 'months'][parseInt(month) - 1];
    };
        ...
        nc(selector).bind("plothover", function (event, pos, item) {
            ...
            if (item) {
                ...
                if (typeof item.series.pie === "undefined" || item.series.pie.show === false) {
                    ...
                    $tooltip.html(html)
                        .css({top: item.pageY - y_offset, left: item.pageX - x_offset})
                        .fadeIn(200);
                } else {
                    $tooltip.html(item.series.label + ": " + item.datapoint[1][0][1].toFixed(2))
                        .css({top: pos.pageY - y_offset, left: pos.pageX + x_offset})
                        .fadeIn(200);
                }
            } else {
                $tooltip.hide();
