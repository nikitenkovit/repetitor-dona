...
// quotes added into the message_put.php!
        ...
        if (!empty($fl)) {
            $real_value = $nc_core->page->fetch_page_metatags("http://".$nc_core->DOMAIN_NAME.nc_folder_path($message));
...
function nc_subdivision_show_edit_form($sub_id, $view) {
    ...
    foreach ($views as $view) {
        echo "<div id='nc_subdivision_form_{$view}_div'" . ($_REQUEST['isNaked'] && $view != 'design' ? " class='nc_class_none'" : "") . ">";
...
?>
    ...
    <div class='nc_admin_form_menu' style='padding-top: 20px;'>
            ...
            <div id='nc_object_slider_menu' class='slider_block_2' style='padding-top: 0px; padding-bottom: 15px;'>
                <ul>
                    <li id='nc_subdivision_form_edit'><?=SUBDIVISION_TAB_INFO_TOOLBAR_EDIT_EDIT?></li>
                    <li id='nc_subdivision_form_design' class='button_on'><?=SUBDIVISION_TAB_INFO_TOOLBAR_EDIT_DESIGN?></li>
...
function nc_subdivision_form_seo($sub_env, $have_default = true) {
    ...
    if ($have_default) {
        ...
        $mt = $nc_core->page->fetch_page_metatags("http://".$domain.nc_folder_path($sub_env['Subdivision_ID']));
        // title
...
function nc_subdivision_form_seo_save() {
    ...
    $component = new nc_Component(0, 2);
    $subdivision_fields = $component->get_fields();
    $subdivision_fields[] = $smo_image = $component->get_smo_image_field();

    foreach ($subdivision_fields as $value) {
        $name = 'f_'.$value['name'];
        global $$name;
        if ($value['type'] == 6) {
            global ${$name."_old"};
            global ${"f_KILL".$value['id']};
        }
    }
    ...
    // проверка
    $real_value = $nc_core->page->fetch_page_metatags("http://".$nc_core->DOMAIN_NAME.nc_folder_path($sub_id));
    foreach ($metatags as $v) {
