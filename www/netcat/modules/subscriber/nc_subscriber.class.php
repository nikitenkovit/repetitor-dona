...
class nc_subscriber {
 ...
 // массив с подписками пользователя
     ...
     *        0 - текущий пользователь. Если нет и его - создается псевдоползователья.
    ...
    public function subscription_add($mailer_id, $user_id = 0, $period = 0, $status = false, $fields = array(), $message = 0) {
        ...
        // текущий пользователь не авторизирован или игнорируется, нужно создать псевдопользователя
        if ($user_id <= 0) {
    ...
    public function get_mailer_by_cc($cc, $item = '') {
        ...
        if (!$mailer_id) {
            ...
            if ($mailer_temp['Class_ID']) $mailer_temp['Sub_Class_ID'] = $cc;
    ...
    public function is_subscribe_to_cc($cc, $user_id = 0) {
        ...
        foreach ($all_subscribers as $v) {
            ...
            if ($cc == $mailer_cc || $class_id  == $mailer_class_id) return true;
        }
     ...
     * @return string текст
     */
    public function replace_macrovariables($text, $current_user, $mailer_id, $reset = 0, $macro = null) {
        ...
        if (empty($con_sub[$mailer_id])) {
            ...
            $con_sub[$mailer_id] = $this->tools->get_subscribe_sub('', $catalogue);
        ...
        $text = str_replace('%LINK%', 
            "http://".$con_sub[$mailer_id]['Domain'].$this->core->SUB_FOLDER.$this->core->HTTP_ROOT_PATH."modules/subscriber/confirm.php?hash=".$links[$current_user['User_ID']][$mailer_id],
            $text);
        $text = str_replace('%SUB_LINK%',
            "http:" . $con_sub[$mailer_id]['url'] . ($current_user['UserType'] == 'pseudo' ? '?auth_hash='.$current_user['Auth_Hash'] : ''),
            $text);
