...
class nc_subscriber_send {
 ...
 // номера подписок, которые участвовали в рассылке
    ...
    public function send_mail($to, $body, $subject, $isHtml, $files = array(), $attachment_type = '') {
        ...
        $nc_core->mail->setCharset($this->_settings['Charset'] ? $this->_settings['Charset'] : MAIN_EMAIL_ENCODING);
        $nc_core->mail->clear();
        ...
        $nc_core->mail->send($to, $this->_settings['FromEmail'], $this->_settings['ReplyEmail'], $subject, $this->_settings['FromName']);
        // количество отправленных писем
        $this->_mail_count++;
