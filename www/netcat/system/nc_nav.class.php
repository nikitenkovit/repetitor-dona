...
class nc_nav extends nc_System {
    ...
    public function get() {
        ...
        // Получаем список файлов, принадлежащих полученным разделам
        $files = array();
        ...
        foreach ($data as $sub_id => $row) {
            ...
            foreach ($file_fields as $field_id => $field_name) {
                if (!empty($row[$field_name])) {
                    ...
                    if (isset($files[$sub_id][$field_id])) {
                        $file_data = $files[$sub_id][$field_id];
                    ...
                    // Original file system
                        ...
                        $row[$field_name] = $row[$field_name . '_url'] = $file_base_url . $f[3];
