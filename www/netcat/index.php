...
$nc_main_content = '';
$statusChanged = $nc_core->input->fetch_get('statusChanged');
$statusCc = $nc_core->input->fetch_get('statusCc');

switch (true) {
    case $use_multi_sub_class && !$cc_keyword:
        if (!empty($statusChanged)) {
            $current_cc = $nc_core->sub_class->set_current_by_id($statusCc);
            $nc_main_content .= nc_objects_list($sub, $statusCc, $nc_core->url->get_parsed_url('query').( isset($date) ? "&date=".$date : "")."&isMainContent=1&isSubClassArray=1");
        } else {
